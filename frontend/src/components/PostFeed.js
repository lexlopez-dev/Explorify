...
const PostFeed = (props) => {
    ...
    useEffect(() => {
        const checkCount = () => {
            ...
            apiCalls.loadNewPostCount(topPostId, props.user)
                .then(response => {
    ...
    const onClickLoadMore = () => {
        ...
        apiCalls.loadOldPosts(postAtBottom.id, props.user)
            .then(response => {
    ...
    const onClickModalOk = () => {
        ...
        apiCalls.deletePost(postToBeDeleted.id)
            .then((response) => {
                setPage(previousPage => ({
                    ...previousPage,
    ...
    const onClickLoadNew = () => {
        ...
        apiCalls.loadNewPosts(topPostId, props.user)
            .then(response => {
                setPage(previousPage => ({
                    ...previousPage,
        ...
        <div>
            {newPostCount > 0 && (
                <div className="card card-header text-center"
                    onClick={!isLoadingNewPosts && onClickLoadNew}
                    style={{ cursor: isLoadingNewPosts ? 'not-allowed' :'pointer'}}
                >
            ...
            {page.last === false && (
                <div className="card card-header text-center"
                    onClick={!isLoadingOldPosts && onClickLoadMore}
                    style={{ cursor: isLoadingOldPosts ? 'not-allowed' :'pointer'}}
                >
            ...
            <Modal 
                ...
                body={postToBeDeleted && `Are you sure you want to delete '${postToBeDeleted.content}'?`} 
                title="Delete"
